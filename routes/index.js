            ...
            campground.find().where('author.id').equals(profile._id).exec((err, campgrounds) => {
                user.find({}, (err, users) => {
                    if (err) {
                        ...
                        res.render("users/show", { profile: profile, campgrounds: campgrounds, users: users });
                    }
                })
...
router.delete("/users/:id", (req, res) => {
    comment.deleteMany({ "author.id": req.params.id }, (err) => {
        if (err) {
            req.flash("error","Unsuccessful,try again later,Sorry for inconvenience");
            req.redirect("/");
        } else {
            campground.deleteMany({ "author.id": req.params.id }, (err) => {
                if (err) {
                    req.flash("error","Unsuccessful,try again later,Sorry for inconvenience");
                    req.redirect("/");
                } else {
